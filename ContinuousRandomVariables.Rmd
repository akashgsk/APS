...
```{r}
f_y = function(x){
  return((1/sqrt(2 * pi * (4^2))) * exp(-(x-198)^2 / (2*4^2)))
  }
```

```{r}
#plot(x,y)
df = as.data.frame(cbind(x,f_x(x),f_y(x))) 
colnames(df) = c('x','f_x(x)','f_y(x)')
```

```{r}
p = ggplot(data = df) +
     geom_line(aes(x = x,y = f_x(x), color = 'Male Students')) +
     geom_line(aes(x = x,y = f_y(x), color = 'Unknowns')) +
     scale_color_manual(name='Students', values = c('Male Students' = 'maroon', 'Unknowns' = 'darkblue')) +
     labs(x = 'Height(cm)',y = 'Density') 
p

```
```{r}
dnorm(190,170,8) #Gives same result as f_x
```
...
p = ggplot(data = df) +
     ...
     geom_line(aes(x = x,y = dnorm(x,190,4), color = 'Unknowns')) +
     scale_color_manual(name='Students', values = c('Male Students' = 'red', 'Unknowns' = 'blue')) +
     labs(x = 'Height(cm)',y = 'Density') 
p
...
p =0.2
j=3
dbinom(j,n,p)  # X~Bin(n=10,p=0.2), p(X=3) -> Exact 3, is Probability Mass Function(PMF)
pbinom(j,n,p)  # p(X <= 3) -> Atmost 3, is Cummulative Distributive Function(CDF)
```
```{r}
...
sigma = 8
x = 165
dnorm(x, mu, sigma) #Density, not Probability, f_x(165) = ProbabilityDensity(prob/cm) at 165cm
```

```{r}
pnorm(x, mu, sigma) #Probability, f_x(165) = P(X <=165cm)
```
```{r}
pnorm(173, mu, sigma) - pnorm(169, mu, sigma) #Random student height between 169-173 cm
```
```{r}
pnorm(x, mu, sigma, lower.tail = FALSE)  #P(X >= 165 cm)
